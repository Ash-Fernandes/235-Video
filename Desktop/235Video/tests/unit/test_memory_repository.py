...
from movies.domain.model import User, Article, Tag, Comment, make_comment
...
def test_repository_can_retrieve_article(in_memory_repo):
    ...
    comment_one = [comment for comment in article.comments if comment.comment == 'Oh no, COVID-19 has hit New Zealand'][
        0]
    comment_two = [comment for comment in article.comments if comment.comment == 'Yeah Freddie, bad showcase'][0]
