...
def test_movies_without_ranking(client):
    ...
    assert b'Amy Adams' in response.data
    assert b'Arrival' in response.data


def test_articles_with_ranking(client):
    # Check that we can retrieve the articles page.
    ...
    assert b'Mila Kunis' in response.data
...
def test_articles_with_genre(client):
    ...
    assert b'Drama/Thriller' in response.data
    assert b'Nocturnal Animals' in response.data
